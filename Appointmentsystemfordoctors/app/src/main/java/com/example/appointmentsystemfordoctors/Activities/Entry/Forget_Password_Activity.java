...
import com.example.appointmentsystemfordoctors.Activities.Firebase.DB_ChangePassword;
...
import com.example.appointmentsystemfordoctors.Activities.Tools.Strings_Tools;
                    ...
                    public void onDataChange(@NonNull DataSnapshot snapshot) {
                        ...
                        if (userName_ID.charAt(0) == 'p') {
                            if (snapshot.child(PATIENTS).child(Strings_Tools.createNOTCleanUserName(userName_ID,1,":")).exists()) {
                                ...
                                Toast.makeText(Forget_Password_Activity.this, "Wrong details", Toast.LENGTH_LONG).show();
                        ...
                        forgetPassword_MainText.setText("Please choose new password");
                        forgetPassword_MainText.setTextSize(25);
                        userName_ID_input_and_password_1.setText("");
                        email_input_and_password_2.setText("");
                        userName_ID_input_and_password_1.setHint("Enter new password");
                        email_input_and_password_2.setHint("Re enter new password");
                        send_emailAndUserID_and_newPassword.setText("Re enter new password");
                    }
 ...
 */
            ...
            public void onClick(View v) {
                if (PERMISSION_TO_CHANGE_PASSWORD == 1) {
                    ...
                    if (!newPassword_1.equals(newPassword_2)) {
                        Toast.makeText(Forget_Password_Activity.this, "The password must be identical", Toast.LENGTH_LONG).show();
                        email_input_and_password_2.setError("This is a mandatory field");
                        return;
                    } else {
                        updatePASSWORD(newPassword_2);
                    }
                } else {
                    Toast.makeText(Forget_Password_Activity.this, "Please try again, the details are wrong", Toast.LENGTH_LONG).show();
                    goBackToLogin_Activity();
                    return;
                }
            }
    ...
    private void updatePASSWORD(String newValidPassword) {
        ...
        Toast.makeText(Forget_Password_Activity.this, "The password had been successfully changed", Toast.LENGTH_LONG).show();
        goBackToLogin_Activity();
        return;
    }
